...
def build_parser(commands) -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("install", help="install skills via runners or browse/fetch the skills.sh registry")
    p.add_argument("source", nargs="?", help="source e.g. vercel-labs/agent-skills or owner/repo/skill")
    ...
    p.add_argument("--trust-confirmed", action="store_true", help="with --preview/fetch commit: confirm the reviewed snapshot and target")
    p.add_argument("--registry-hash", metavar="HEX", help="with --preview/fetch: registry content hash for change detection")
    p.add_argument("--review", metavar="REVIEW_ID", help="with --fetch: commit a previously fetched registry review without another network request")
    registry_ops = p.add_mutually_exclusive_group()
    registry_ops.add_argument("--browse", action="store_true", help="browse one skills.sh leaderboard page")
    registry_ops.add_argument("--search", metavar="QUERY", help="search the skills.sh catalog")
    registry_ops.add_argument("--curated", action="store_true", help="list the skills.sh curated catalog")
    registry_ops.add_argument("--fetch", action="store_true", help="fetch one skills.sh skill for review, or commit it with --review")
    p.add_argument("--page", type=int, default=0, help="registry leaderboard page (default: 0)")
    p.add_argument("--per-page", type=int, default=25, help="registry leaderboard page size (default: 25)")
    p.add_argument("--view", choices=("all-time", "trending", "hot"), default="all-time", help="registry leaderboard view")
    p.add_argument("--allow-stale", action="store_true", help="use an expired registry cache if the network is unavailable")
    p.add_argument("--json", action="store_true")
