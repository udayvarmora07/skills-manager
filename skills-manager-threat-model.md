...
**Date:** 2026-09-20 · **Version:** 1.5 · **Scope:** `skills-mgr` CLI + local web UI (`skillsmgr/webapp.py`, `skillsmgr/webui/`), `Store` + `scopes` data layer, plus the build/release configuration its artifacts depend on. **1.5** records the resolved ADR-004 policy: HMAC shared-secret integrity only, file-only offline distribution, and content-only signed metadata; archive verification remains unimplemented. Earlier versions add the supply-chain paths closed by audit batch 5 — a sha256-pinned vendored bundle, a hash-verified pinned build toolchain, no index install under a write-capable release job, and an archive gate that inspects every member (`SEC-7`, `SEC-8`, `SEC-9`, `SEC-13`, all `FIXED`) — plus `T-18`/`R-9`, the `script-src 'unsafe-eval'` trade-off recorded as accepted rather than silently relaxed (`SEC-4`).
...
| T-13 Forged or tampered team bundle accepted as reviewed (ADR-004) | **Policy resolved; archive verification is still unimplemented.** The future design requires an explicit verification gate (a bundle that declares a signature and fails verification is refused before staging, never silently downgraded to unsigned), a canonical MAC input covering member names *and* contents, and a key stored outside the data dir so `export --full`/backup cannot carry it | Not mitigated today because nothing reads a bundle signature; future implementation must not let "verified" imply "safe to run" (ADR-004 §5) |
...
| R-6 | Team bundle signed with a shared secret is mistaken for individual authorship (T-13) | Medium (wording drift is easy) | Low | **POLICY RESOLVED 2026-09-20** — HMAC is group authenticity only; the ADR forbids claiming individual identity and requires that limit in any future UI/doc text |
